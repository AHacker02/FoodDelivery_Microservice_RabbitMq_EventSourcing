            ...
            var filteredRestaurant = from r in restaurants
                                     where (id==null||r.Id==Guid.Parse(id)) 
                                        && (name == null || r.Name.ToLower().Contains(name.ToLower()))
                                           && (location == null || r.Location.ToLower().Contains(location.ToLower()) || r.ListedCity.ToLower().Contains(location.ToLower()) ||
                                               location.ToLower().Contains(r.Location.ToLower()) || location.ToLower().Contains(r.ListedCity.ToLower()))
                                           && (budget == null || r.ApproxCost >= Convert.ToDecimal(budget))
                                           && (rating == null || r.Rating_Calc <= Convert.ToDecimal(rating))
                                           && (food == null || r.Menu.Any(x => x.Item.ToLower().Contains(food.ToLower()) || food.ToLower().Contains(x.Item.ToLower())))
                                     select r;
